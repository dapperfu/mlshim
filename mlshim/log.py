...
def configure_logger(stream_level="DEBUG", debug_file=None):
    ...
    if isinstance(stream_level, int):
        stream_level = LOG_LEVEL_MAP[stream_level]
    # Set up 'mlshim' logger
    logger = logging.getLogger("mlshim")
    logger.setLevel(logging.DEBUG)

    # Remove all attached handlers, in case there was
    # a logger with using the name 'mlshim'
    del logger.handlers[:]
